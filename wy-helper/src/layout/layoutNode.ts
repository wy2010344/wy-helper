 ...
 * 即布局本身确实需要依附一个节点存在
 */
export class LayoutNode<M, K extends string>
  implements MDisplayOut<K>, LayoutModel<K>
{
  target!: M;
  _index: number = 0;
...
export function createLayoutNode<M, K extends string>(
  ...
  const axis = objectMap(n.axis, function (v, key) {
    ...
    return {
      ...
      paddingStart,
      paddingEnd,
      drawSize: getInnerSize(v.size, c, getIns, key, paddingStart, paddingEnd),
    };
  });
