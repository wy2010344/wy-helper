...
export function scrollerSignal(
  p: {
  ...
  function publish(
    ...
    const wasAnimating = __isAnimating;
    if (wasAnimating) {
      wasAnimating();
      __isAnimating = false;
    }
    ...
    if (animate && animating()) {
      ...
      __scheduledTop = _top;
      const oldLeft = left.get();
      const oldTop = top.get();
      const oldZoom = zoomLevel.get();

      const diffLeft = _left - oldLeft;
      const diffTop = _top - oldTop;
  ...
  function scrollBy(_left: number, _top: number, animate?: boolean) {
    const startLeft = __isAnimating ? __scheduledLeft : left.get();
    const startTop = __isAnimating ? __scheduledTop : top.get();
  ...
  function zoomTo(
    ...
    let _left = ((originLeft + left.get()) * level) / oldLevel - originLeft;
    let _top = ((originTop + top.get()) * level) / oldLevel - originTop;
    // Limit x-axis
  ...
  function doTouchStart(
    ...
    const initialTouchLeft = n.currentTouchLeft;
    const initialTouchTop = n.currentTouchTop;
    ...
    function doTouchMove(
      ...
      if (isDragging) {
        // 计算移动距离
        const moveX = n.currentTouchLeft - lastTouchLeft;
        const moveY = n.currentTouchTop - lastTouchTop;

        let scrollLeft = left.get();
        let scrollTop = top.get();
        let level = zoomLevel.get();
        ...
        if (scale && zooming()) {
          ...
          if (oldLevel !== level) {
            ...
            const currentTouchLeftRel = n.currentTouchLeft - client.left;
            const currentTouchTopRel = n.currentTouchTop - client.top;
            // Recompute left and top coordinates based on new zoom level
            scrollLeft =
        ...
        if (enableScrollY) {
          ...
          const _maxScrollTop = maxScrollTop();
        ...
        const minimumTrackingForScroll = locking ? 3 : 0;
        const minimumTrackingForDrag = 5;

        const distanceX = Math.abs(n.currentTouchLeft - initialTouchLeft);
        const distanceY = Math.abs(n.currentTouchTop - initialTouchTop);
    ...
    function doTouchEnd(timeStamp: number) {
      if (isDragging) {
        // 开始减速
        ...
        if (isSingleTouch && animating() && timeStamp - lastTouchMove <= 100) {
          ...
          for (
            let i = endPos;
            i > 0 && positions[i] > lastTouchMove - 100;
            i -= 3
          ) {
            startPos = i;
          }
          ...
          if (startPos !== endPos) {
            ...
            const movedLeft = left.get() - positions[startPos - 2];
            const movedTop = top.get() - positions[startPos - 1];
            ...
            // 验证我们是否有足够的速度来开始减速
            if (
              Math.abs(decelerationVelocityX) >
                minVelocityToStartDeceleration ||
              Math.abs(decelerationVelocityY) > minVelocityToStartDeceleration
            ) {
              // 减速时停用下拉刷新
              if (!refreshActive.get()) {
                ...
                if (paging()) {
                  const scrollLeft = Math.max(
                    Math.min(left.get(), maxScrollLeft()),
                    0
                  );
                  const scrollTop = Math.max(
                    Math.min(top.get(), maxScrollTop()),
                    0
                  );
                  const _clientWidth = clientWidth();
                  const _clientHeight = clientHeight();
                  // 我们将减速限制在允许范围的最小/最大值，而不是可见客户区的大小。
                  // 每个页面应具有与客户区完全相同的大小。
                  __minDecelerationScrollLeft =
                    Math.floor(scrollLeft / _clientWidth) * _clientWidth;
                  __minDecelerationScrollTop =
                    Math.floor(scrollTop / _clientHeight) * _clientHeight;
                  __maxDecelerationScrollLeft =
                    Math.ceil(scrollLeft / _clientWidth) * _clientWidth;
                  __maxDecelerationScrollTop =
                    Math.ceil(scrollTop / _clientHeight) * _clientHeight;
                } else {
                ...
                // 需要多少速度才能保持减速运行
                const minVelocityToKeepDecelerating = snapping() ? 4 : 0.1;
                // 开始动画并打开标志
                __isDecelerating = Animate.start(
                  ...
                  function (percent: number, now: number, render: boolean) {
                    ...
                    // 为滚动位置添加减速
                    //在动画的每个步骤上调用
                    let scrollLeft = left.get() + decelerationVelocityX;
                    let scrollTop = top.get() + decelerationVelocityY;
                    //
                    // 非弹跳模式的硬限制滚动位置
                    //
                    if (!bouncing) {
                      const scrollLeftFixed = Math.max(
                        Math.min(__maxDecelerationScrollLeft, scrollLeft),
                        __minDecelerationScrollLeft
                      );
                      ...
                      const scrollTopFixed = Math.max(
                        Math.min(__maxDecelerationScrollTop, scrollTop),
                        __minDecelerationScrollTop
                      );
                    ...
                    if (!paging()) {
                      ...
                      // 以减慢进程。这应该模拟自然行为，其中
                      // 当移动发起者被移除时，物体会减速
                      const frictionFactor = decelerationRate;
                    ...
                    if (bouncing) {
                      ...
                      // 这配置了到达边界时减速/加速的变化量
                      const _penetrationDeceleration = penetrationDeceleration;
                      const _penetrationAcceleration = penetrationAcceleration;

                      // Check limits
                      if (scrollLeft < __minDecelerationScrollLeft) {
                        scrollOutsideX =
                          __minDecelerationScrollLeft - scrollLeft;
                      } else if (scrollLeft > __maxDecelerationScrollLeft) {
                        scrollOutsideX =
                          __maxDecelerationScrollLeft - scrollLeft;
                      }
                      ...
                      if (scrollOutsideX !== 0) {
                        if (scrollOutsideX * decelerationVelocityX <= 0) {
                          decelerationVelocityX +=
                            scrollOutsideX * _penetrationDeceleration;
                        } else {
                          decelerationVelocityX =
                            scrollOutsideX * _penetrationAcceleration;
                        }
                      }
                      ...
                      if (scrollOutsideY !== 0) {
                        if (scrollOutsideY * decelerationVelocityY <= 0) {
                          decelerationVelocityY +=
                            scrollOutsideY * _penetrationDeceleration;
                        } else {
                          decelerationVelocityY =
                            scrollOutsideY * _penetrationAcceleration;
                        }
                      }
                    }
                  },
                  ...
                  function () {
                    const shouldContinue =
                      ...
                      Math.abs(decelerationVelocityY) >=
                        minVelocityToKeepDecelerating;
                    if (!shouldContinue) {
  ...
  return {
    ...
    left: left.get,
    top: top.get,
    doMouseZoom(wheelDelta: number, clientX: number, clientY: number) {
