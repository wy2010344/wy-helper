...
export function dateFormat(v: Date, format: string) {
  ...
  str = str.replace(
    /dd|DD/,
    v.getDate() > 9 ? v.getDate().toString() : `0${v.getDate()}`
  );
  str = str.replace(
    /hh|HH/,
    v.getHours() > 9 ? v.getHours().toString() : `0${v.getHours()}`
  );
  str = str.replace(
    /mm/,
    v.getMinutes() > 9 ? v.getMinutes().toString() : `0${v.getMinutes()}`
  );
  str = str.replace(
    /ss/,
    v.getSeconds() > 9 ? v.getSeconds().toString() : `0${v.getSeconds()}`
  );
  return str;
}
...
export function getWeekOfYear(date: Date, weekStart = 0) {
  ...
  const daysSinceStart =
    Math.floor((date.getTime() - firstDayOfYear.getTime()) / DAYMILLSECONDS) +
    daysOffset;
...
export function getDayOfYear(date: Date) {
  ...
  return (
    Math.floor((date.valueOf() - startOfYear.valueOf()) / DAYMILLSECONDS) + 1
  );
}
