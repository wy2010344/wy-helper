...
export const isChinese = CharRange.of(
  getCharCode('\u4e00'),
  getCharCode('\u9fa5')
)
...
export const isFloat = andMatch(
  isInteger,
  orMatchEmpty(
    andMatch(matchAnyString('.'), manyMatch(isNumber.matchCharBetween(), 1))
  )
)

export const isFloatWithOpposite = orMatch(
  isFloat,
  andMatch(matchCharIn(getCharCode('-')), isFloat)
)
