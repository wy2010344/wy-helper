...
export function ruleStrBetweenGet1(begin: number, end: number = begin) {
...
export function ruleStrBetween(begin: number, end = begin) {
  ...
  return andMatch(
    matchCharIn(begin),
    manyMatch(
      orMatch(matchAnyString('\\\\'), matchAnyString(`\\${endChar}`), anyChar),
      0,
      matchTheEnd,
      matchTheEnd
    ),
    //可能结束了,但没有闭合
    orMatch(matchEnd, matchCharIn(end))
  );
}

export function ruleStrBetweenGet(begin: number, end = begin) {
  const endChar = String.fromCharCode(end);
  const matchTheEnd = matchToEnd(endChar);
  return andRuleGet(
    [
      ...
      //可能结束了,但没有闭合
      ruleGet(orMatch(matchEnd, matchCharIn(end)), quote),
    ],
    function (a, b, c) {
      return b.join('');
    }
...
export function ruleStrBetweenPart(
  ...
  return andMatch(
    orMatch(matchAnyString(begin), matchAnyString(inEnd)),
    manyMatch(
      orMatch(
        matchAnyString('\\\\'), //+2
    ...
    //可能结束了,但没有闭合
    orMatch(matchEnd, matchAnyString(end), matchAnyString(inBegin))
  );
}
...
export function ruleStrBetweenPartGet(
  ...
  return andRuleGet(
    [
      ruleGet(orMatch(matchAnyString(begin), matchAnyString(inEnd)), quote),
