...
export class WeekVirtualView {
  static from(year: number, month: number, day: number, firstWeek: number) {
    const d = new Date();
    d.setFullYear(year);
    d.setMonth(month - 1);
    d.setDate(day);
  ...
  static fromDate(d: Date, firstWeek: number) {
  ...
  equals(n: WeekVirtualView) {
    return (
      n.firstWeek == this.firstWeek &&
      arrayEqual(n.cells, this.cells, yearMonthDayEqual)
    );
  }
