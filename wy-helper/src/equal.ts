...
export function simpleEqualsEqual<
  T extends {
    equals(b: T): boolean;
  },
>(a: T, b: T) {
...
export function simpleEqualsNotEqual<
  T extends {
    equals(b: T): boolean;
  },
>(a: T, b: T) {
...
export function arrayEqual<T>(
  a1: ReadArray<T>,
  a2: ReadArray<T>,
  equal: (x: T, y: T) => boolean = simpleEqual
) {
  if (a1 == a2) {
    return true;
  }
 ...
 * @param equal
 */
export function arrayEndsWith<T>(
  a1: ReadArray<T>,
  a2: ReadArray<T>,
  equal: (x: T, y: T) => boolean = simpleEqual
) {
...
export function arrayReduceRight<Ts extends readonly any[]>(
  vs: Ts,
  fun: (m: Ts[number], i: number, vs: Ts) => void
) {
...
export function arrayReduceLeft<Ts extends readonly any[]>(
  vs: Ts,
  fun: (m: Ts[number], i: number, vs: Ts) => void
) {
  vs.forEach(fun as any);
}
export function buildRemoveWhere<T, M>(
  equal: (m: M, a: T, idx: number) => any
) {
...
export const removeWhere = buildRemoveWhere(function <T>(
