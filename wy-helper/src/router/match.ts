...
const ruleGetKey = ruleGet<Que, string>(
  manyMatch(matchNotChatColon, 1),
  ruleGetString
);
/**
 * [abc]
...
const getQuote = andRuleGet(
  [
    ...
    orRuleGet([
      andRuleGet(
        [ruleGet(matchCharIn(Colon), quote), ruleGetKey],
        function (a, b) {
          return b;
        }
      ),
...
const parseRule = andRuleGet(
  [
    manyRuleGet(
      andRuleGet([getStr0, getQuote], function (a, b) {
        return {
          begin: a,
          key: b[0],
          type: b[1],
    ...
    if (typeof match == 'string') {
      ...
      if (query.startsWith(match.before, idx)) {
        ...
        if (afterFlag) {
          ...
          if (nextIdx < 0) {
            throw new Error(
              `${query.slice(match.before.length + idx)}未找到${afterFlag}的分割`
            );
          }
        ...
        throw new Error(
          `${query.slice(match.before.length + idx)}不是以${match.before}开始`
        );
      }
    }
  }
...
export function toMatchNode(
  node: string,
  typeDefMap: Record<string, (v: string) => any>
) {
