...
export function buildUseExitAnimate<V>(
  ...
  function buildRemove(old: ExitModelInner<V>) {
    ...
    promise.then(function () {
      //是删除,不考虑needMerge
      const eCacheList =
        cacheList.renderHelper || new ArrayHelper(cacheList.list);
  ...
  function opHelperExiting(
    eCacheList: NoInsertArrayHelper<ExitModelInner<V>>,
    thisRemoveList: ExitModelInner<V>[]
  ) {
    let n = 0;
  ...
  for (let i = 0, len = list.length; i < len; i++) {
    ...
    const oldIndex = newCacheList
      .get()
      .findIndex(old => !isExitingItem(old) && old.originalKey == key);
    if (oldIndex < 0) {
      if (mode == 'shift') {
  ...
  return {
    ...
    effect() {
      const removePromiseList: Promise<any>[] = thisRemoveList.map(
        v => v.promise
      );
...
function hideAsShowAndRemoteHide<T>(
  v: ExitModelInner<T>,
  i: number,
  array: ArrayHelper<ExitModelInner<T>>
) {
