...
import {
  ReducerDispatch,
  ReducerWithDispatchResult,
  createReduceValueCenter,
  mapReducerDispatch,
} from '../ValueCenter';
...
export class PromiseStopCall<T> {
  ...
  reloadBack(value: VersionPromiseResult<T>) {
    if (value.version == this.requestVersion) {
      return new PromiseStopCall(this.requestVersion, undefined, value);
    }
    return this;
  }
  reload(getFun: () => Promise<T>) {
    ...
    return [new PromiseStopCall(version, c, this.data), act] as const;
  }

  setData(fun: (old: T) => T) {
    if (this.data?.type == 'success') {
      return new PromiseStopCall(this.requestVersion, this.abortController, {
        ...this.data,
...
export class PromiseAutoLoadMore<T, K> {
  constructor(
    ...
    private readonly getFun:
      | ((k: K) => Promise<AutoLoadMoreCore<T, K>>)
      | undefined = undefined,
  ...
  reload(
    ...
    return [
      new PromiseAutoLoadMore(data, getFun, getKey, false),
      function (dispatch) {
  ...
  reloadBack(v: VersionPromiseResult<AutoLoadMoreCore<T, K>>) {
    ...
    return new PromiseAutoLoadMore(u, this.getFun, this.getKey, false);
  }

  private loadMoreList(
    version: number,
    list: ReducerDispatch<VersionPromiseResult<AutoLoadMoreCore<T, K>>>[]
  ) {
    if (!this.getFun) {
      return this;
    }
  ...
  loadMore(version: number) {
    const list: ReducerDispatch<
      VersionPromiseResult<AutoLoadMoreCore<T, K>>
    >[] = [];
    const data = this.loadMoreList(version, list);
    return [data, arrayFunToOneOrEmpty(list)] as const;
  }
  ...
  refresh(getFun: () => Promise<AutoLoadMoreCore<T, K>>) {
    const that = this;
    const [data, act] = this.data.reload(getFun);
    return [
      new PromiseAutoLoadMore(data, this.getFun, this.getKey, false),
      function (dispatch) {
        that.loadMoreAbortController?.abort();
        act(dispatch);
      } as typeof act,
...
type AutoModel<T, K> = ReducerWithDispatchResult<
...
export function valueCenterAutoLoadMore<T, K>() {
  return createReduceValueCenter<
    PromiseAutoLoadMore<T, K>,
    AutoLoadMoreAction<T, K>
  >(reducerAutoLoadMore, PromiseAutoLoadMore.empty as any);
}
