...
export function forceManybody<T>({
  ...
  return function (
    ...
    const tree = (
      g.nDim === 1
        ? binarytree(nodes, getD('x'))
        : g.nDim === 2
          ? quadtree(nodes as any[], getD('x'), getD('y'))
          : g.nDim === 3
            ? octree(nodes, getD('x'), getD('y'), getD('z'))
            : null
...
function apply(
  ...
  const x2 = [arg1, arg2, arg3][g.nDim - 1];

  let x = treeNode.x - g.node.x.d,
  ...
  if ((w * w) / g.theta2 < l) {
    if (l < g.distanceMax2) {
      ...
      if (g.nDim > 1) {
        g.node.y.v += (y * treeNode.value * g.alpha) / l;
      }
      if (g.nDim > 2) {
        g.node.z.v += (z * treeNode.value * g.alpha) / l;
      }
    }
    return true;
  }
