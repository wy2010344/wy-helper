...
interface MergeNodesAndLinks<N, L, K, FDir extends ForceDir = ForceDir> {
  (arg: {
    ...
    createForceNode(
      n: N,
      i: number,
      befores: readonly ForceNode<N, FDir>[]
    ): ForceNode<N, FDir>;
  }): {
    nodes: readonly ForceNode<N, FDir>[];
    ...
    if (fnode) {
      ...
      if (node) {
        ...
        if (key == fkey) {
          ...
          if (targetIndex < 0) {
            //新的
            helper.insert(i, createForceNode(fnode, i, nodes));
          } else {
            //旧的
...
export function mergeNodes<N, K, FDir extends ForceDir = ForceDir>(
  arg: MergeNodesArg<N, K, FDir>
) {
  return _mergeNodes(arg).get();
}
export interface MergeNodesAndLinksArg<
  N,
  L,
  K,
  FDir extends ForceDir = ForceDir,
> extends MergeNodesArg<N, K, FDir> {
  links: readonly ForceLink<L, N, FDir>[];
  fromLinks: ReadArray<L>;
  getNodeKey(n: N): K;
  getSorceKey(n: L): K;
...
export function mergeNodesAndLinks<N, L, K, FDir extends ForceDir = ForceDir>(
  arg: MergeNodesAndLinksArg<N, L, K, FDir>
) {
...
let cloneDir = quote;
function cloneNode<T, FDir extends ForceDir>(
  node: ForceNode<T, FDir>,
  index: number
): ForceNode<T, FDir> {
  return {
    index,
...
function simpleCloneLinks<T, V, FDir extends ForceDir>(
  links: readonly ForceLink<V, T, FDir>[],
