...
export function springBase(
  ...
  if (zta < 1) {
    ...
    const displacement =
      underDampedEnvelope * (cosCoeff * cos1 + sinCoeff * sin1);
    return {
      //FM,Gpt,Jc,ReA
      displacement,
      //Jc,Rea依赖上一步,没有对上,Gpt,FM没有提供
      velocity:
        displacement * -omega0 * zta +
    ...
    const coeffA =
      deltaX -
      (gammaMinus * deltaX - initialVelocity) / (gammaMinus - gammaPlus);
    const coeffB =
      (gammaMinus * deltaX - initialVelocity) / (gammaMinus - gammaPlus);
    return {
      //Jc
      displacement:
        coeffA * Math.exp(gammaMinus * elapsedTime) +
        coeffB * Math.exp(gammaPlus * elapsedTime),
      //Jc
      velocity: velocityWhenZta1Plus
        ? coeffA * gammaMinus * Math.exp(gammaMinus * elapsedTime) +
          coeffB * gammaPlus * Math.exp(gammaPlus * elapsedTime)
        : 0,
    };
  }
}
 ...
 */
export function springIsStop(
  n: SpringOutValue,
  displacementThreshold = globalDefaultDisplacementThreshold,
  velocityThreshold = 10
) {
  return (
    Math.abs(n.displacement) < displacementThreshold &&
    Math.abs(n.velocity) < velocityThreshold
  );
}
