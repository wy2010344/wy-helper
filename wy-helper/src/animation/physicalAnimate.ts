...
export function scrollInfinityIteration(
  ...
  return function (out: SilentDiff) {
    ...
    let accValue = 0;
    return physicalAnimationFrame(function () {
      if (Math.abs(velocity) < minVelocityThreshold) {
        return true;
      }
...
function scrollEdgeIterationFn(
  ...
  minVelocityThreshold =
    Math.abs(minVelocityThreshold) || defaultMinVelocityThreshold;
  return function (out: SilentDiff) {
...
export async function scrollEdgeIteration(
  ...
  let backTarget = 0,
    backVelocity = 0,
    then = false;
  const value = await scroll.change(
    scrollEdgeIterationFn(arg, function (target, velocity) {
      backTarget = target;
