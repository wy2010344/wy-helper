...
export class ClampingScrollFactory {
  ...
  static get(
    drag: number = 2,
    displacementThreshold: number = globalDefaultDisplacementThreshold
  ) {
    return new ClampingScrollFactory(drag, displacementThreshold);
  }

  static edgeClampingConfig(
    velocity: number,
    displacementThreshold = globalDefaultDisplacementThreshold
  ) {
    return ClampingScrollFactory.get(100, displacementThreshold)
      .getFromVelocity(velocity)
      .animationConfig();
  }
...
export class ClampingScroll implements ScrollHelper {
  ...
  // 除以1000,变成秒
  getDisplacement(elapsedTime: number) {
    return (
      this.distance * (1 - Math.exp((-this.factory.drag * elapsedTime) / 1000))
    );
  }
  ...
  getTimeToDistance(value: number) {
    return (
      (-Math.log(1 - (value * this.factory.drag) / this.velocitySecond) *
        1000) /
      this.factory.drag
    );
  }
  ...
  getVelocity(elapsedTime: number) {
    return (
      this.initVelocity * Math.exp((-this.factory.drag * elapsedTime) / 1000)
    );
  }
