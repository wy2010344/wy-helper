...
export function pointEqual<T>(a: Point<T>, b: Point<T>, eq: Compare<T> = simpleEqual) {
  if (a == b) {
    return true
  }
...
export function pointOperate<T, F>(a: Point<T>, b: Point<T>, op: (a: T, b: T) => F): Point<F> {
  return {
    x: op(a.x, b.x),
    y: op(a.y, b.y)
  }
...
export function axisEqual<T>(a: Axis<T>, b: Axis<T>, eq: Compare<T> = simpleEqual) {
  if (a == b) {
    return true
  }
...
export function boxEqual<T>(a: Box<T>, b: Box<T>, eq: Compare<T> = simpleEqual) {
  if (a == b) {
    return true
  }
...
export function boundingBoxEqual<T>(a: BoundingBox<T>, b: BoundingBox<T>, eq: Compare<T> = simpleEqual) {
  if (a == b) {
    return true
  }
  return eq(a.top, b.top) && eq(a.bottom, b.bottom) && eq(a.left, b.left) && eq(a.right, b.right)
}
