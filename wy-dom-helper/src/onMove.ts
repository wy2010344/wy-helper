...
export function subscribeMove(
  ...
  const moveOption: AddEventListenerOptions = {};
  const endOption: AddEventListenerOptions = {};
  const cancelOption: AddEventListenerOptions = {};
  const c = arg?.capture;
  if (c) {
    if (typeof c == 'boolean') {
...
export function subscribeDragMove(onMove: DragMove, arg?: OnMoveArg) {
  const dm = subscribeMove(
    function (e, a) {
...
export function subscribeDragInit(
  div: Node,
  fun: (e: PagePoint, ole: Event) => void,
  capture?: boolean
) {
  const cancelOption: AddEventListenerOptions = {
    capture,
  };
  const d1 = subscribeEvent(
    div,
    'mousedown',
    function (e: any) {
      fun(e, e);
