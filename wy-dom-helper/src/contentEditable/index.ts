...
export function contentInput(
  ...
  const idx = beforeText.length + addIdx;
  return {
    ...args,
    ...
    if (shiftKey) {
      if (beforeText.endsWith(tab)) {
        ...
        return {
          ...args,
          value:
            beforeText.slice(0, beforeText.length - tab.length) + afterText,
          range: {
            start: newIndex,
            end: newIndex,
          },
...
export const contentEditableText =
  browser.type == 'FF' ? true : ('plaintext-only' as const);
...
export function appendRecord(
  ...
  if (last && last.range.start == last.range.end) {
    ...
    if (record.range.start == record.range.end) {
      ...
      if (0 <= diff) {
        if (
          record.value.startsWith(last.value) &&
          record.value.length - last.value.length == diff
        ) {
          //当前对之前是纯包含关系,直接替换
          history.pop();
          history.push(record);
